...
async function callAssetLLMChunked(args) {
  ...
  async function worker() {
    for (;;) {
      ...
      if (text) {
        // 道具禁止子状态：偶发被模型写成「#### @名称」子标题时规格化成 ### 顶级小节。
        if (chunk.type === 'props') text = text.replace(/^####\s*@?/gm, '### ');
        // 场景允许日/夜等 #### 子状态；但若模型漏写 ### 分组（全文只有 ####），
        // 按「@基础名_状态」的基础名自动补 ### 分组，防止多个场景粘成一段。
        if (chunk.type === 'scenes' && !/(^|\n)###\s/.test(text) && /(^|\n)####\s/.test(text)) {
          const groups = new Map();
          for (const b of text.split(/\n(?=####\s)/)) {
            const mm = b.match(/^####\s*@?([^\n_]+)/);
            const base = mm ? mm[1].trim() : '场景';
            if (!groups.has(base)) groups.set(base, []);
            groups.get(base).push(b.trim());
          }
          text = [...groups.entries()].map(([base, arr]) => `### ${base}\n${arr.join('\n\n')}`).join('\n\n');
        }
